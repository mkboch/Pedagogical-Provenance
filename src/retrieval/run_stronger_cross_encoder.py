...
results={}
for policy in ["truncate512","chunked_max"]:
...
sweep.to_csv(OUT/"stronger_reranker_k_sweep.csv",index=False)
...
mrr.to_csv(OUT/"stronger_reranker_mrr.csv",index=False)
json.dump({"model":MODEL,"baseline_model":BASE_MODEL,"subsample_stride":SUBSAMPLE_STRIDE,
          ...
          open(OUT/"stronger_reranker_metadata.json","w"),indent=2)
